...
extern __constant__ double gpu_R;
extern __constant__ double gpu_gamma;
extern __constant__ double gpu_dx;
extern __constant__ double gpu_dt;
...
Euler::Euler(int size, double t) {
  ...
  cudaMalloc((void **)&(gpu_data.u), size * sizeof(double));
  cudaMalloc((void **)&(gpu_data.p), size * sizeof(double));
  cudaMalloc((void **)&(gpu_data.E), size * sizeof(double));
  cudaMalloc((void **)&(gpu_data.T), size * sizeof(double));
  cudaMalloc((void **)&(gpu_data.c), size * sizeof(double));
  /*gpu_u*/
  cudaMalloc((void **)&(gpu_U.u1), size * sizeof(double));
  cudaMalloc((void **)&(gpu_U.u2), size * sizeof(double));
  cudaMalloc((void **)&(gpu_U.u3), size * sizeof(double));
  /*set constant memory*/
  cudaMemcpyToSymbol(gpu_R, &R0, sizeof(double));
  cudaMemcpyToSymbol(gpu_gamma, &gamma, sizeof(double));
  cudaMemcpyToSymbol(gpu_dx, &dx, sizeof(double));
  cudaMemcpyToSymbol(gpu_dt, &dt, sizeof(double));
  /*event created*/
...
void Euler::memCpy() {
  ...
  cudaMemcpy(tmp, gpu_data.x, size * sizeof(double), cudaMemcpyDeviceToHost);
  for (int i = 0; i < size; i++)
    data[i].x = tmp[i];
  cudaMemcpy(tmp, gpu_U.u1, size * sizeof(double), cudaMemcpyDeviceToHost);
  for (int i = 0; i < size; i++)
    U[i].rou_r = tmp[i];
  cudaMemcpy(tmp, gpu_U.u2, size * sizeof(double), cudaMemcpyDeviceToHost);
  for (int i = 0; i < size; i++)
    U[i].rou_u = tmp[i];
  cudaMemcpy(tmp, gpu_U.u3, size * sizeof(double), cudaMemcpyDeviceToHost);
  for (int i = 0; i < size; i++)
    U[i].rou_e = tmp[i];
  free(tmp);
  refresh();
